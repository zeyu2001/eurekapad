...
export class SingletonJSRunner {
  ...
  public async runJS(code: string, language: string, stdout: (_msg: string) => void, stderr: (_msg: string) => void) {
    if (!this.loaded) {
      throw new Error('JS runner is not loaded yet')
    }

    // Ensure that only one execution is running at a time
    this.idle = this.idle.then(() => this._runJS(code, language, stdout, stderr))
    return this.idle
  }

  private async _runJS(code: string, language: string, stdout: (_msg: string) => void, stderr: (_msg: string) => void) {
    try {
      ...
      return new Promise<void>((resolve, _reject) => {
        sendMessage(new AsyncFunction(transpiledCode).toString(), [], (error, result, done) => {
          if (error) {
