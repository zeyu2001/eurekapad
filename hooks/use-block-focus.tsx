...
const findFirstFocusableElement = (container: HTMLElement) => {
  return (Array.from(container.getElementsByTagName('*')) as HTMLElement[]).find(isFocusable)
}
...
export function useBlockFocus<T extends CustomBlockConfig, I extends InlineContentSchema, S extends StyleSchema>(
  ...
  setTimeout(function poll() {
    if (ref.current && !firstFocusable) {
      const focusable = isFocusable(ref.current) ? ref.current : findFirstFocusableElement(ref.current)
